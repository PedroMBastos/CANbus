...
u_int8_t receiveCAN(CAN_HANDLE fd0, Message * m)
{
  int res;
...
/***************************************************************************/
u_int8_t sendCAN(CAN_HANDLE fd0, Message const * m)
{
  int res;
  struct can_frame frame;
  ...
  if (res < 0)
    {
      // Commented out as part of axof: 13299
...
u_int8_t canChangeBaudRate( CAN_HANDLE fd, char* baud)
{
...
/***************************************************************************/
CAN_HANDLE openCAN (s_PORT * board)
{
  struct ifreq ifr;
